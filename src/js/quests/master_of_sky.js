...
define(['phaser', 'helper', 'objects/tree_evaluator', 'quests/quest'], 
    ...
    function MasterOfSky(game, gui, treeEvaluator, tier) {
        ...
        this.size = Helper.lerp(0.6,0.1,this.difficulty);
        this.condition = {
            sum_lower: Math.floor(Helper.lerp(20,5,this.difficulty)),
            sum_upper: Math.floor(Helper.lerp(25,40,this.difficulty))
        };
        this.description = "Grow a tree, with only the upper half of the screen filled with leafs.";
        this.description += "\nYou need at least " + this.condition.sum_upper + " leaves in the upper part";
        this.description += "\nYou can have at most " + this.condition.sum_lower + " leaves in the lower part";
    ...
    MasterOfSky.prototype.progressMsg = function() {
        var msg = "";
        msg += Math.floor(this.evaluation.sum_upper) + " / " + this.condition.sum_upper + " in upper part\n";
        msg += Math.floor(this.evaluation.sum_lower) + " / " + this.condition.sum_lower + " in lower part";
        return msg;
    };
