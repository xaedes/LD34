...
define(['phaser', 'helper'], function (Phaser, Helper) {
    function LeafSprites(game, tree) {
        ...
        this.leafs_per_frame_min = 5;
        this.leafs_per_frame_max = 10;
        ...
        for (var frame_idx = 0; frame_idx < this.num_frames; ++frame_idx) {
            ...
            for(var k = 0; k < leafs_per_frame; ++k) {
                ...
                var cx = this.game.rnd.integerInRange(0, this.colormap.width);
                var cy = this.game.rnd.integerInRange(0, this.colormap.height);
                leaves.push({
                    leaf: this.generateLeaf(this.leaf_width, this.leaf_height, 0.25, this.padding + rx, this.padding + ry),
                    color: this.pickColor(cx,cy),
                    cx: cx,
                    cy: cy
                });
            }
    ...
    LeafSprites.prototype.treeUpdate = function () {
        ...
        this.tree.traverseBranches(function(branch){
            branch.leafs.forEach(function (leaf) {
                for(var anim_idx = 0; anim_idx < this.anim_len; ++anim_idx) {
                    this.leaf.frame = (leaf.frame_idx % this.num_frames) * this.anim_len + anim_idx;

                    this.drawingTexture.renderRawXY(this.leaf,
                        branch.x + leaf.x * this.leaf_displacement_x + anim_idx * this.game.width,
    ...
    LeafSprites.prototype.pickColor = function (x,y) {
        return this.colormap.getPixel32(x, y);
    };
